...
assessed. This will be helpful, to gain a better understanding of riverine heatwaves. Applying these methods on a dataset from Main river system in Bavaria delivered a significant increase in frequency over time, but only insignificant increasing temporal trends in duration and intensity. Also assumed trend between river kilometer and the heatwave metrics could not be verified. However, the dataset only included a relatively short time period, therefore a larger data size might lead to more precise results.
...
analysis on a data set of water temperature data from Main river system, focusing on the evolution of riverine 
...
depicted below, where y is the year variable, d is the day variable and j is the day for the considered temperature \@ref(eq:threshold). $Y_s$ and $y_e$ are the years that mark the beginning and end of the baseline period, $P_{90}(X)$ is the 90 percent 
...
heatwave \@ref(eq:duration) [@hobday2016]. The duration plays an important role, as especially longer heatwaves can have more severe
impacts.
...
river, and while the mean stays consistent, the variance seems to increase downstream \@ref(fig:mean-duration-overview). Also extremely long heatwaves 
...
For the temporal trend a similar approach was used. The yearly mean durations for each station were plotted \@ref(fig:duration-plot), and for each a linear model was fitted. Four of the six stations showed a positive trend, with only one of them being 
...
and for the differences between stations in figure \@ref(fig:polygon-spatial). From the literature a increasing trend 
over time for all three metrics would be expected. This analysis only reflects a significant increasing trend for 
