...
# The Ember source data is not redistributed with the book (see
# work/11-positive_tp/data/README.md), so positive_tipping_analysis.R cannot be
# sourced here. The chapter renders from the artefacts that script produced:
# figures/*.png and results/*.csv.
ptpFig <- function(name) {
  knitr::include_graphics(file.path("work/11-positive_tp/figures", name))
}
ptpResult <- function(name) {
  read.csv(file.path("work/11-positive_tp/results", name))
}
```
...
selected_model_table <- ptpResult("selected_models.csv")
...
```{r ptp-ewssummary}
ews_summary_table <- ptpResult("ews_summary.csv")
