...
The data are processed sequentially, one time step after another, as illustrated in Figure ... (in this study, one time step corresponds to one day). At each step, an LSTM cell receives the current input together with the hidden and cell state from the previous time step:
...
Figure ... illustrates the computations within an LSTM cell.
...
Via a variation of Partial Dependence Plots (PDPs), investigating how manipulating a feature's entire 365-day history by adding a constant offset $a$ or multiplying each day with a constant factor $a$ affects the predicted water temperature. all else kept unchanged.
...
The permutation itself differs between the two feature types, as illustrated in Figure .... For a static feature $S_j$, the observation $s_j^{(1)}, \dots, s_j^{(n)}$ are permuted randomly across observations. For a dynamic feature $X_j$, the time series $x_j^{(i, 0)}, \dots, x_j^{(i, 364)}$ is permuted within each observation $i$. In both cases, the following three streps are then applied:
