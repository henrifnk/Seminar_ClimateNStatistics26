...
```{r irh-img-function, echo=FALSE}
path_figures <- "work/06-interpretable_heat/figures"
include_img <- function(img_name, dir = path_figures) {
  ext <- if (knitr::is_latex_output()) "pdf" else "svg"
  knitr::include_graphics(file.path(dir, paste0(img_name, ".", ext)))
}
```
...
```{r irh-fig-stations-map, fig.cap="Map of the 14 measurement stations along the river Main and its tributaries Regnitz, Fränkische Saale, Itz and Wern. Map created with Felt and River Data from Natural Earth Data."}
knitr::include_graphics(file.path(path_figures, "images/stations_map.png"))
```
...
```{r irh-var-cols, results='asis', echo=FALSE}
...
```{r irh-fig-stations-periods, fig.cap="Temporal coverage of each measurement station.", out.width="100%"}
include_img("stations_periods")
```
...
```{r irh-fig-stations-missing-patterns, fig.cap=" Missing Patterns of dynamic variables on a daily basis for each station. A daily value is missing if at least one of the eight three-hourly observations for the respective variable and station is missing.", fig.show='hold'}
include_img("stations_missings")
```
...
```{r irh-fig-corr-vars-heatmap, fig.cap="Heatmap of pairwise correlations between all variables. For dynamic variables, it shows the correlations of the daily synchronized values of all available days.", out.width="80%"}
include_img("corr_vars_heatmap")
```
...
A Long-Short-Term Memory (LSTM) network is a recurrent neural network (RNN) architecture designed for sequential data, such as time series [@zhang2023]. LSTMs mitigate the vanishing-gradient problem of conventional RNNs by incorporating a separate cell state with gated information flow. This allows information to be retained or discarded over longer time intervals, enabling the model to capture temporal dependencies.

```{r irh-fig-lstm-seq-proc, fig.cap="Sequential processing of the input data by an LSTM. Each cell represents one time step and receives the current input as well as the hidden and cell states from the previous time step."}
include_img("images/lstm_sequential_processing")
```
...
```{r irh-fig-lstm-cell-structure, fig.cap="Structure of an LSTM cell. The current input and previous hidden state are combined and passed through the forget, input and output gates and the input node to update the cell and hidden states."}
include_img("images/lstm_cell_structure")
```
...
```{r irh-fig-tensor-feature-perm, fig.cap="Manipulation of the static feature matrix and dynamic feature tensor for the calculation of Permutation Feature Importance. For static features, the corresponding feature values are permuted across all observations. For dynamic features, the time series of the corresponding feature is permuted within each observation.", out.width="80%"}
include_img("images/tensor_feature_permutation")
```
...
```{r irh-fig-feat-imp, fig.cap="Permutation Feature Importance of static (orange) and dynamic (blue) features."}
include_img("int_perm_importance")
```
...
```{r irh-fig-pdp, fig.cap="Partial Dependence Plots for Air Temperature and Solar Radiation. Each gray line represents the ICE curve of an individual observation, that is, how that observation's prediction would change under the corresponding offset or factor, while the red line shows the averaged PDP curve."}
include_img("int_pdp")
```
...
```{r irh-fig-lag-importance, fig.cap="Lag Importance for different Lags of Air Temperature and Solar Radiation. Note that the y-axes of the two plots are scaled differently."}
include_img("int_lag_importance")
```
...
```{r irh-fig-pdp-2d, fig.cap="Two-dimensional Partial Dependence Plot of Air Temperature and Solar Radiation", out.width="80%"}
include_img("int_pdp2d_Ta_C_rad_whm2")
```
