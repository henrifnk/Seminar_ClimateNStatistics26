# Neural Hydrology {#neural-hydrology}
...
## Empirical Study: Regional Transfer Learning Under Data Scarcity {#case-study}
...
```{r transfer-setup, include=FALSE}
...
transfer_benchmark <- read.csv("work/04-neural_hyd/results/results_folder_groups_summary_with_global_true_ea.csv", stringsAsFactors = FALSE)
transfer_all_basins <- read.csv("work/04-neural_hyd/results/results_folder_groups_all_basins_ea.csv", stringsAsFactors = FALSE)

transfer_group_cuda <- read.csv("work/04-neural_hyd/results/results_folder_groups_summary_with_global_true.csv", stringsAsFactors = FALSE)
transfer_all_cuda <- read.csv("work/04-neural_hyd/results/results_folder_groups_all_basins.csv", stringsAsFactors = FALSE)

g05_local_ft <- read.csv("work/04-neural_hyd/results/results_folder_05_local_vs_finetune_ea.csv", stringsAsFactors = FALSE)
g05_global <- read.csv("work/04-neural_hyd/results/results_folder_05_global_eval_true_ea.csv", stringsAsFactors = FALSE)

g01_local_ft_cuda <- read.csv("work/04-neural_hyd/results/results_folder_01_local_vs_finetune.csv", stringsAsFactors = FALSE)
g01_global_cuda <- read.csv("work/04-neural_hyd/results/results_folder_01_global_eval_true.csv", stringsAsFactors = FALSE)
...
g05_local_mean <- g05_summary$local_nse_mean[1]
g05_global_mean <- g05_summary$global_nse_mean[1]
g05_ft_mean <- g05_summary$ft_nse_mean[1]
g05_ft_local_delta <- g05_summary$delta_ft_vs_local_nse[1]
g05_ft_global_delta <- g05_summary$delta_ft_vs_global_nse[1]
...
Table \@ref(tab:transfer-overall-table) reports basin-weighted overall metrics.

```{r transfer-overall-table, echo=FALSE}
overall <- data.frame(
...
```{r transfer-by-group-nse, fig.height=4.8, fig.width=10.0}
transfer_long <- rbind(
...
```{r transfer-delta-bars, fig.height=4.2, fig.width=10.0}
gain_long <- rbind(
...
```{r transfer-basin-box-ecdf, fig.height=4.2, fig.width=10.0}
basin_df <- data.frame(
...
```{r transfer-basin-ecdf, fig.height=4.0, fig.width=10.0}
ecdf_df <- data.frame(
  nse = c(transfer_all_basins$local_nse, transfer_all_basins$ft_nse),
  model = factor(c(rep("Local", nrow(transfer_all_basins)), rep("Fine-tune", nrow(transfer_all_basins))), levels = c("Local", "Fine-tune"))
)
...
```{r transfer-map-by-group-book, fig.height=4.8, fig.width=10.8}
topo <- read.csv(
...
```{r transfer-coverage-bars-book, fig.height=3.8, fig.width=9.8}
...
```{r transfer-group-size-dist-book, fig.height=2.8, fig.width=6.6}
...
```{r transfer-overall-nse-bars-book, fig.height=3.3, fig.width=8.2}
...
This bar chart restates Table \@ref(tab:transfer-overall-table) in visual form and makes
the between-strategy NSE gaps easier to compare.

```{r transfer-overall-kge-bars-book, fig.height=3.3, fig.width=8.2}
...
```{r transfer-delta-kge-bars-book, fig.height=4.1, fig.width=9.6}
dk_long <- rbind(
...
```{r transfer-local-ft-scatter-book, fig.height=3.8, fig.width=8.9}
...
```{r transfer-local-vs-gain-book, fig.height=3.6, fig.width=8.9}
...
```{r transfer-arch-nse-book, fig.height=3.4, fig.width=10.0}
arch_nse <- data.frame(
...
```{r transfer-group05-table-book, results='asis'}
g05_cmp <- data.frame(
...
```{r transfer-group05-bars-book, fig.height=4.2, fig.width=10.4}
