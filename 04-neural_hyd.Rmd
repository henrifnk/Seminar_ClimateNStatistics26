...
temperature seasonality, and snowmelt timing all shift over time [@nh-ipcc2021].
...
3. **Non-stationarity**  -  parameters calibrated under historical climate conditions may
  not hold under future climate forcing [@nh-milly2008].
...
The pivotal step came in 2018 when @nh-kratzert2018 demonstrated that a standard
Long Short-Term Memory (LSTM) network, trained on the
**CAMELS-US** dataset [@nh-addor2017] of 241 catchments, outperformed all
...
The LSTM [@nh-hochreiter1997] maintains a **cell state** $\mathbf{c}_t$ as an internal
...
@nh-kratzert2019 extended the single-basin LSTM to **simultaneous multi-basin training**
...
**NeuralHydrology** [@nh-kratzert2022joss] is an open-source Python library developed at
...
| **EA-LSTM** | Entity-Aware LSTM | Static features condition input gate [@nh-kratzert2019] |
| **MC-LSTM** | Mass-Conserving LSTM | Precipitation mass budget enforced [@nh-hoedt2021] |
| **MTS-LSTM** | Multi-Timescale LSTM | Joint training at daily and hourly resolution [@nh-gauch2021] |
...
| **Mamba** | State-space model | Linear-time sequence modelling [@nh-gu2023] |
| **xLSTM** | Extended LSTM | Exponential gating [@nh-beck2024] |
...
[@nh-milly2008]. A model trained on historical data may fail under future conditions if it
...
continental United States, or multi-country datasets like Caravan [@nh-kratzert2023caravan]
...
@nh-frame2022 showed that LSTMs pre-trained on multi-basin data and fine-tuned on a single
...
@nh-hoedt2021 argue that **physically consistent architectures** (such as MC-LSTM) are
