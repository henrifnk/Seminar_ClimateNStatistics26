...
Layout:
    ...
    └── diagnostics/   the plot_diffusion_process scripts
        ├── forward_process_<date>/
        ├── denoiser_estimate_<date>_seed<seed>/   (one step_<NN>/ subfolder per step)
        └── ensemble_spread_<source>_<date>_n<N>.svg   (a file, not a folder)
"""
