"""Generate reports/output/results_report.html — self-contained with embedded images.

Baseline (naive/naive architecture, loss-selection grid):
...
FIG      = ROOT / "figures"        # baselines/, processed/, models/<tag>/, frontier plots
OUT      = ROOT / "reports" / "output" / "results_report.html"
...
# Baseline = naive/naive arch; architecture sweep = all other arch conditions.
# The CSV has no "phase" column — classify by static_encoder / global_encoder.
_baseline_all  = [r for r in _rows_all
                if r.get("static_encoder","") == "naive"
                and r.get("global_encoder","") == "naive"]
_baseline_ok     = [r for r in _baseline_all if _has_all(r)]
_baseline_excl   = [r for r in _baseline_all if not _has_all(r)]


def _baseline_winners(complete_rows):
...
print(f"Baseline rows in CSV    : {len(_baseline_all)}")
print(f"All required cols OK    : {len(_baseline_ok)}")
print(f"Excluded (missing cols) : {len(_baseline_excl)}")
if _baseline_excl:
    for r in _baseline_excl:
        missing = [c for c in _REQUIRED if _safe(r.get(c)) is None]
        print(f"  EXCLUDED: {r['run_name']}  missing: {', '.join(missing)}")

# One winner per loss group — baseline (naive/naive)
rows = _baseline_winners(_baseline_ok)
print(f"Baseline winners shown  : {len(rows)}  "
      f"({', '.join(r['run_name'] for r in rows)})")

# ── Architecture-sweep winners — one per (loss, dw, static_enc, global_enc) ─
_arch_all = [r for r in _rows_all
...
_arch_groups: dict = {}
for r in _arch_all:
    _arch_groups.setdefault(_arch_group_key(r), []).append(r)

_arch_winners = [
    sorted(v, key=lambda r: _safe(r.get("val/loss")) or float("inf"))[0]
    for v in _arch_groups.values()
]
print(f"Arch-sweep winners loaded : {len(_arch_winners)}  ({len(_arch_groups)} groups, {len(_arch_all)} rows total)")

# Combined lookup pool: baseline naive/naive + architecture-sweep conditions
all_winners = rows + _arch_winners
...
# Baseline uses only these four families (naive/naive architecture).
...
def _cell_runs(loss, static_enc, global_enc, mode=None, drought_weight=None):
    """Winner row for one (loss, condition) cell — searches baseline + architecture-sweep pool."""
    out = [
...
EVAL_RUNS: set[str] = (
    {d.name[: -len("_eval")] for d in (FIG / "models").iterdir()
     if d.is_dir() and d.name.endswith("_eval")}
    if (FIG / "models").exists() else set()
)


def model_fig(run_name, metric):
    return FIG / "models" / f"{run_name}_eval" / f"test_{metric}_spatial.png"
...
<title>Drought Prediction — Loss &amp; Architecture Results</title>
<style>{CSS}</style></head>
<body><div class="container">
<h1>Alpine Drought Prediction — Loss &amp; Architecture Results</h1>
<p class="subtitle">ConvLSTM · 12-month lead · SPEI regression · Test period 2015–2024 ·
  Baseline: {len(rows)} loss-group winners (naive/naive) ·
  Architecture sweep: {len(_arch_winners)} architecture-condition winners (5 conditions × 4 loss groups × best-val/loss HP)</p>
""")
...
if len(_baseline_excl) > 0:
    excl_list = "<br>".join(r["run_name"] for r in _baseline_excl)
    W(f'<div style="background:#2d1e0e;border-left:3px solid #f87171;padding:12px 16px;'
      f'border-radius:0 6px 6px 0;margin:12px 0">'
      f'<p style="color:#fde68a;margin:0"><strong>Excluded runs ({len(_baseline_excl)}) — '
...
W('<h2 id="rq1">RQ1 — Loss Function Comparison (Baseline)</h2>')
...
try:
    ...
    _tpr_img = _scatter_b64(
        ...
        save_path=FIG / "frontier_tpr.png",
        save_light_path=FIG / "frontier_tpr_light.png",
    )
    _f1_img = _scatter_b64(
        "test/drought_f1_pooled", "F1", FRONT_F1,
        save_path=FIG / "frontier_f1.png",
        save_light_path=FIG / "frontier_f1_light.png",
    )
    W(f'<div class="card"><img src="{_tpr_img}" style="width:100%;border-radius:4px">'
      f'<p class="fig-label">rmse_vs_trend vs pooled TPR — {len(all_winners)} winners (baseline + arch sweep), '
    ...
    W(f'<div class="card"><img src="{_f1_img}" style="width:100%;border-radius:4px">'
      f'<p class="fig-label">rmse_vs_trend vs pooled F1 — {len(all_winners)} winners (baseline + arch sweep), '
