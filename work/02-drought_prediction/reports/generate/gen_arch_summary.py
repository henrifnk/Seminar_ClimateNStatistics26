"""Generate reports/output/arch_summary.html — baseline + architecture-sweep summary.

Reads from saved_models/gridsearch_comparison.csv.

Baseline: one best-val/loss winner per (loss_fn, drought_weight) group — naive/naive arch.
Architecture sweep: all 4 loss groups × 5 arch conditions × 8 HP each (160 runs).
         Winners selected by val/loss within each (loss_fn, drought_weight, arch) group.

Run:
    uv run python reports/generate/gen_arch_summary.py
"""
...
ROOT     = Path(__file__).parent.parent.parent  # reports/generate/ -> reports/ -> project root
CSV_PATH = ROOT / "saved_models" / "gridsearch_comparison.csv"
OUT      = ROOT / "reports" / "output" / "arch_summary.html"
...
# Baseline = naive/naive architecture (loss-selection grid); architecture sweep =
# the other 5 architecture conditions. Classified by static_encoder/global_encoder
# (present in every CSV row) rather than a "phase" column or run_name string
# matching — both are fragile against renaming and WandB never logged a
# top-level "phase" hparam into the comparison CSV.

# ── Baseline — one winner per (loss_fn, drought_weight) by val/loss ─────────
baseline_all = [r for r in rows_all
          if r.get("static_encoder", "") == "naive"
          and r.get("global_encoder", "") == "naive"
          and _has_all(r)]

def _baseline_group_key(r):
    loss = r.get("loss_fn", "")
...
for r in baseline_all:
    baseline_groups[_baseline_group_key(r)].append(r)

baseline_winners: dict = {
    k: sorted(v, key=lambda r: _safe(r.get("val/loss")) or float("inf"))[0]
    for k, v in baseline_groups.items()
}
print(f"Baseline winners : {len(baseline_winners)}")

# ── Architecture sweep — winner per (loss, dw, arch) by val/loss ────────────
arch_all = [r for r in rows_all
          if not (r.get("static_encoder", "") == "naive" and r.get("global_encoder", "") == "naive")
          and _has_all(r)]

def _arch_group_key(r):
    loss = r.get("loss_fn", "")
    dw   = r.get("drought_weight", "") if loss == "weighted_mse" else ""
    return (loss, dw, r.get("static_encoder", ""), r.get("global_encoder", ""))

arch_groups: dict = defaultdict(list)
for r in arch_all:
    arch_groups[_arch_group_key(r)].append(r)

arch_winners: dict = {
    k: sorted(v, key=lambda r: _safe(r.get("val/loss")) or float("inf"))[0]
    for k, v in arch_groups.items()
}
print(f"Architecture-sweep groups : {len(arch_groups)}  ({len(arch_all)} rows total)")
for k in sorted(arch_groups):
    print(f"  {k}: {len(arch_groups[k])} rows")

# ── Baselines ─────────────────────────────────────────────────────────────────
_b = next(iter(baseline_winners.values()))
...
# ── Display config ────────────────────────────────────────────────────────────
# Baseline: (loss_fn, dw|None, label, color, bg)
_BASELINE_DISPLAY = [
...
# Architecture-sweep loss groups (same 4, reused for section headings)
_ARCH_LOSS_GROUPS = [
    ("mse",          "", "MSE",            "#5599ff"),
...
def _find_baseline(loss, dw):
    return baseline_winners.get((loss, dw), {})

def _find_arch(loss, dw, static, global_):
    return arch_winners.get((loss, dw, static, global_), {})
...
def _baseline_card(r, label, color, bg):
...
def _baseline_bar_row(r, label, color, max_f1, max_rmse, kind):
    if kind == "f1":
...
def _baseline_table_row(r, label, color, bg, f1_vals, rmse_vals):
...
def _arch_section(loss, dw, label, accent):
    """Generate full architecture-sweep subsection for one loss group."""
    ref = _find_baseline(loss, dw)
    arch_rows = [
        (s, g, lbl, col, bg, _find_arch(loss, dw, s, g))
    ...
    ref_row = ""
    if ref:
        chip = '<span style="font-family:var(--mono);font-size:12px;color:var(--muted)">naive / naive</span><span style="font-size:10px;color:var(--muted);margin-left:6px">Baseline ref</span>'
...
def _scatter_data():
    pts = []
    for loss, dw, label, color, _ in _BASELINE_DISPLAY:
        r   = _find_baseline(loss, dw)
    ...
    pts.append('  { name: "Trend baseline", x: 0.0000, y: 0.0000, r: 5, color: "#4e5e7a", ref: true }')
...
# ── Architecture-sweep scatter data ───────────────────────────────────────────
...
def _arch_scatter_data():
    pts = []
    for s, g, arch_lbl, col, bg in _ARCH_DISPLAY:
        for loss, dw, loss_lbl, loss_col in _ARCH_LOSS_GROUPS:
            r = _find_arch(loss, dw, s, g)
            if not r:
                continue
            rvt = _safe(r.get("test/rmse_vs_trend")) or 0.0
            f1  = _safe(r.get("test/drought_f1_pooled")) or 0.0
    ...
    # Baseline refs (greyed out, for comparison)
    for loss, dw, loss_lbl, loss_col, _ in _BASELINE_DISPLAY:
        r = _find_baseline(loss, dw)
        if not r:
            continue
        rvt = _safe(r.get("test/rmse_vs_trend")) or 0.0
        f1  = _safe(r.get("test/drought_f1_pooled")) or 0.0
        pts.append(f'  {{ name: {("Baseline " + loss_lbl)!r}, x: {rvt:.4f}, y: {f1:.4f}, '
                   f'r: 5, color: {loss_col!r}, ref: true }}')
    return "[\n" + ",\n".join(pts) + "\n]"


# ── Compute baseline summary values ─────────────────────────────────────────

baseline_data      = [(_find_baseline(lf, dw), lbl, col, bg) for lf, dw, lbl, col, bg in _BASELINE_DISPLAY]
baseline_f1_vals   = [_safe(r.get("test/drought_f1_pooled")) for r, *_ in baseline_data]
baseline_rmse_vals = [_safe(r.get("test/rmse_pooled"))       for r, *_ in baseline_data]
max_baseline_f1    = max(v for v in baseline_f1_vals  if v is not None)
max_baseline_rmse  = max(v for v in baseline_rmse_vals if v is not None)

baseline_cards     = "".join(_baseline_card(r, lbl, col, bg)          for r, lbl, col, bg in baseline_data)
baseline_f1_bars   = "".join(_baseline_bar_row(r, lbl, col, max_baseline_f1, max_baseline_rmse, "f1")   for r, lbl, col, _ in baseline_data)
baseline_rmse_bars = "".join(_baseline_bar_row(r, lbl, col, max_baseline_f1, max_baseline_rmse, "rmse") for r, lbl, col, _ in baseline_data)
baseline_table_rows = "".join(_baseline_table_row(r, lbl, col, bg, baseline_f1_vals, baseline_rmse_vals) for r, lbl, col, bg in baseline_data)

# Architecture-sweep sections for all 4 loss groups
arch_sections = "".join(_arch_section(loss, dw, label, accent) for loss, dw, label, accent in _ARCH_LOSS_GROUPS)

scatter_data    = _scatter_data()
arch_scatter_data = _arch_scatter_data()

all_baseline_rvt  = [_safe(r.get("test/rmse_vs_trend")) for r, *_ in baseline_data if _safe(r.get("test/rmse_vs_trend")) is not None]
persist_rvt = TREND_RMSE - PERSIST_RMSE
x_min = min(min(all_baseline_rvt), persist_rvt) - 0.06
x_max = max(all_baseline_rvt) + 0.06
y_max = max(baseline_f1_vals + [PERSIST_F1]) * 1.20 + 0.02

# Architecture-sweep scatter bounds: all 20 winners
_arch_rvt_vals = [_safe(r.get("test/rmse_vs_trend")) for r in arch_winners.values() if _safe(r.get("test/rmse_vs_trend")) is not None]
_arch_f1_vals  = [_safe(r.get("test/drought_f1_pooled")) for r in arch_winners.values() if _safe(r.get("test/drought_f1_pooled")) is not None]
arch_x_min = min(_arch_rvt_vals + all_baseline_rvt + [persist_rvt]) - 0.06
arch_x_max = max(_arch_rvt_vals + all_baseline_rvt) + 0.06
arch_y_max = max(_arch_f1_vals + baseline_f1_vals) * 1.15 + 0.02
...
.section-pill{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;padding:2px 8px;
...
html = f"""<title>Loss &amp; Architecture Results</title>
<style>{CSS}</style>
...
<div class="wrap">
  ...
  <div class="hdr">
    <div class="run-meta">
      <span class="dot"></span>
      Baseline · naive/naive · 32 runs → 4 winners &nbsp;·&nbsp;
      Architecture sweep · 4 losses × 5 archs × 8 HP = 160 runs → 20 winners
    </div>
    <h1>Loss Function &amp; Architecture Results</h1>
    <p class="sub">
      Baseline: one best-val/loss winner per loss group on naive/naive architecture.
      Architecture sweep: full HP grid per (loss × architecture) condition, winner by val/loss.
  ...
  <div class="section-heading">
    <span class="section-pill">Baseline</span>
    <h2>Loss function winners — naive/naive architecture</h2>
  </div>

  <div class="cards">{baseline_cards}
  </div>
  ...
  <h2>Drought skill vs RMSE (Baseline)</h2>
  ...
  <!-- ── Architecture sweep ─────────────────────────────────────────────────── -->
  <div class="section-heading">
    <span class="section-pill">Architecture sweep</span>
  ...
  <p style="margin-top:-12px;margin-bottom:28px;font-size:12px;color:var(--muted)">
    ...
    naive/naive is the baseline row at matching HP — not rerun.
  </p>

  <div class="p2-grid">{arch_sections}
  </div>
  <p style="margin-top:-20px;margin-bottom:40px;font-size:11px">
  ...
  <h2>Drought skill vs RMSE (Architecture sweep — all winners)</h2>
  <div class="scatter-wrap">
    <p style="margin-bottom:16px;font-size:12px">
      x: trend RMSE − model RMSE (positive = beats trend) · y: drought F1 ·
      colour = loss family · baseline references shown faded
    </p>
    ...
    <div style="display:flex;flex-wrap:wrap;gap:16px;margin-top:14px;font-size:11px">
      ...
      <span style="color:var(--muted)">Faded = baseline naive/naive reference</span>
    </div>
  </div>
  ...
  <!-- ── Findings ───────────────────────────────────────────────────────────── -->
  <div class="decision">
    <h2>Architecture-sweep findings</h2>
    <ul>
      <li>
        <strong>Pinball: all architectures improve on the baseline.</strong>
        Every architecture-sweep condition beats the naive/naive ref (F1 = 0.384). single/film and naive/film lead
      ...
      <li>
        <strong>WMse w=1: architecture-sweep conditions regress below the baseline ref.</strong>
        naive/naive (F1 = 0.277) beats all 5 architecture-sweep conditions after proper HP selection.
      ...
      <li>
        ...
        single/film (F1 = 0.275) nearly matches the baseline ref (0.297). seasonal/naive drops to
      ...
      <li>
        ...
        With the single baseline HP, naive/film (wmse w=1) had F1 = 0.096.
...
(function() {{
  ...
  const C = isDark
    ...
    : {{bg:'#ffffff',grid:'#d0daf0',text:'#8892b0',zero:'#8892b0'}};
  canvas.style.background = C.bg;
  const data = {arch_scatter_data};
  const xMin={arch_x_min:.3f}, xMax={arch_x_max:.3f}, yMin=-0.02, yMax={arch_y_max:.3f};
  const tx = v => PAD.left+(v-xMin)/(xMax-xMin)*cw;
  const ty = v => PAD.top+(1-(v-yMin)/(yMax-yMin))*ch;
  ctx.strokeStyle=C.grid; ctx.lineWidth=1; ctx.setLineDash([3,4]);
  for (let y=0; y<={arch_y_max:.1f}; y=Math.round((y+.1)*100)/100) {{
