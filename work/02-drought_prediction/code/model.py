...
from utils.metrics import drought_rmse_pooled, metrics_celled_drought, metrics_celled_regression
...
class WeightedMSELoss(nn.Module):
    """MSE with per-cell hinge weights that upweight negative SPEI values.

    weight = 1 + drought_weight * max(0, -target) — flat at 1 for positive
    SPEI, linearly increasing for negative SPEI. Only the dry tail is
    upweighted. drought_weight controls the slope (the final grid uses 1 and
    5: at drought_weight=1, SPEI=-1.5 → weight=2.5, SPEI=-2.0 → weight=3.0).

    (Two other weighting modes — 'threshold': binary weight below
    drought_threshold; 'absolute': weight=|target| on both tails — were
    explored but are not part of the final 24-config grid; see README.)
    """
...
class RCNNModule(LightningModule):
    def __init__(
        ...
        # Checkpoints saved before the classification head (final_classify, boundaries)
        # was removed still have those keys in their state_dict. Non-strict loading
        # skips them and keeps every regression-relevant weight, which is unaffected.
