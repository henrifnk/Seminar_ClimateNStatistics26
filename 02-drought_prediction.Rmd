...
```{r dp-setup, message=FALSE, warning=FALSE, include=FALSE}
library(bookdown)
library(svglite)
library(kableExtra)
```
...
<style>
.book-body .page-inner p { text-align: justify; hyphens: auto; }
...
# For PDF, use the .png twin of each figure (LaTeX can't embed .svg); HTML keeps the .svg.
dp_img <- function(p) knitr::include_graphics(
  if (knitr::is_latex_output()) sub("\\.svg$", ".png", p) else p
)
# Emit raw HTML only for HTML output (empty string for LaTeX).
h <- function(x) if (knitr::is_html_output()) x else ""
# out.width in CSS px for HTML (the two-column CSS grid layout the div
# wrappers above set up). PDF has no such grid -- true side-by-side
# minipages don't survive pandoc's own paragraph/display-math block
# splitting -- so LaTeX instead gets a modest, centered, stand-alone
# figure sized as a fraction of the full text width.
ow <- function(px, latex_frac = 0.33) if (knitr::is_latex_output()) paste0(round(100 * latex_frac), "%") else paste0(px, "px")
```

## Introduction

Drought has no single, universally accepted definition, and which definition applies depends on the part of the water cycle and the impacts under consideration [@zargar2011].
Four types are commonly distinguished (Figure \@ref(fig:dp-drought-types), adapted from @nandgude2023): in a simplified manner a precipitation deficit produces a *meteorological* drought, which depletes soil moisture into an *agricultural* drought, lowers streamflow and groundwater into a *hydrological* drought, and finally reaches water-dependent goods as a *socioeconomic* drought.

(ref:dp-drought-types-cap) The four drought types as a propagating cascade. The depiction is adapted from @nandgude2023.

```{r dp-drought-types, echo=FALSE, fig.cap="(ref:dp-drought-types-cap)", out.width="100%", fig.align="center"}
dp_img("work/02-drought_prediction/figures/general/drought_types_vector.svg")
```
...
It can be quantified with various indices, of which this work uses the Standardized Precipitation Evapotranspiration Index (SPEI) [@vicenteserrano2010].
...
Drought prediction has moved from simple baselines such as logistic regression and gradient-boosted trees (see e.g. @marusov2024) toward machine and deep learning [@nandgude2023].
...
Models from the LSTM family capture the temporal side well but treat space only through their inputs or not at all [@wang2023; @chen2025], whereas graph-based and transformer models represent the spatial field more directly [@yu2023; @gao2023; @pathak2022; @nguyen2023], at the cost of model scale and compute, and without a guaranteed performance edge, since simple linear baselines can often rival transformer forecasters [@zeng2022].

Across the named studies two factors stand out.
First, they mostly forecast at a short, one-month lead [@wang2023; @chen2025; @yu2023], the main exception treating long-lead drought as a binary classification [@marusov2024].
...
A fixed mask marks the valid Alpine cells, shown outlined against the surrounding European topography in Figure `r if (knitr::is_latex_output()) "\\@ref(fig:dp-al-topography-latex)" else "\\@ref(fig:dp-al-topography-html)"`.

```{r dp-al-topography-html, echo=FALSE, fig.cap="Topography over Europe. The red outline marks the Alpine study domain.", out.width="70%", fig.align="center", eval=knitr::is_html_output()}
dp_img("work/02-drought_prediction/figures/raw/topography.svg")
```
...
The *dynamic spatial* variables, which vary in both space and time. They include the SPEI (the target, also supplied as history, also seen in Figure \@ref(fig:dp-spei-snapshots)), precipitation, near-surface temperature, surface pressure, and water balance.

The *dynamic global* variables, which vary in time only and capture the large-scale drivers of European moisture and its seasonality: the North Atlantic Oscillation index [@hurrell1995], grouped Mediterranean sea-surface temperatures (SST), and a cyclical month encoding.
The fourteen Mediterranean basins are aggregated into three regional means [@millot2005; @lionello2006] (Figure \@ref(fig:dp-med-slt)).

```{r dp-static-spatial-html, echo=FALSE, results='asis'}
if (knitr::is_html_output()) cat("And the *static spatial* input, which varies in space only: the topography, whose elevation strongly shapes Alpine precipitation and temperature.\n")
```
...
```{r dp-al-topography-latex, echo=FALSE, fig.cap="Topography over Europe. The red outline marks the Alpine study domain.", out.width="70%", fig.align="center", fig.pos="htbp", eval=knitr::is_latex_output()}
dp_img("work/02-drought_prediction/figures/raw/topography.svg")
```

```{r dp-spei-snapshots, echo=FALSE, fig.cap="SPEI over the Alpine domain across 1971.", out.width="100%", fig.align="center", fig.pos="htbp"}
dp_img("work/02-drought_prediction/figures/raw/al/spei.svg")
```

```{r dp-med-slt, echo=FALSE, fig.cap="Mediterranean SST, 1971--2024, aggregated into three regional groups (western Mediterranean, eastern Mediterranean, Black Sea).", out.width="100%", fig.align="center", fig.pos="htbp"}
dp_img("work/02-drought_prediction/figures/raw/slides/mediterranean_slt.svg")
```
...
The Standardized Precipitation Evapotranspiration Index (SPEI) [@vicenteserrano2010] extends the precipitation-only Standardized Precipitation Index (SPI) [@mckee1993] by using the water balance, so that temperature enters through evapotranspiration, which can better account for rising temperatures under climate change.
...
The index is computed as in @funk2026, with a three-parameter gamma fit calibrated over 1990 to 2019.
...
```{r dp-drought-freq-severe, echo=FALSE, fig.cap="Severe-drought (SPEI $\\le -1.5$) frequency by split.", out.width="100%", fig.align="center"}
dp_img("work/02-drought_prediction/figures/processed/exploratory/drought_frequency_severe.svg")
```
...
The forecasting model is a Convolutional LSTM (ConvLSTM), following the variant of @marusov2024.
It replaces the matrix products in the gates of a standard LSTM [@hochreiter1997] with convolutions, so the input $x_t$, hidden state $h_t$, and cell state $c_t$ are three-dimensional feature maps rather than vectors, which preserves spatial structure.
Figure \@ref(fig:dp-convlstm-schematic) shows the resulting cell.

(ref:dp-convlstm-schematic-cap) ConvLSTM cell. The depiction is by @marusov2024.

```{r dp-convlstm-schematic, echo=FALSE, fig.cap="(ref:dp-convlstm-schematic-cap)", out.width="75%", fig.align="center", fig.pos="htbp"}
dp_img("work/02-drought_prediction/figures/architecture/ConvLSTM.svg")
```
...
`r h('<div style="display: grid; grid-template-columns: 1fr 400px; align-items: center; gap: 1.5em; border-left: 5px solid #3C3C3C; padding-left: 1em;">')`
...
`r h('</div>')`
`r h('<div class="loss-fig" style="min-width: 0; width: 360px; text-align: right;">')`
```{r dp-mse-illustration, echo=FALSE, out.width=ow(170), fig.align=if (knitr::is_latex_output()) "center" else "default"}
dp_img("work/02-drought_prediction/reports/figures/mse.svg")
```
`r h('</div>')`
`r h('</div>')`

`r h('<div style="display: grid; grid-template-columns: 1fr 400px; align-items: center; gap: 1.5em; border-left: 5px solid #286EA0; padding-left: 1em;">')`
`r h('<div style="min-width: 0;">')`
...
`r h('</div>')`
`r h('<div class="loss-fig" style="min-width: 0; width: 360px; text-align: right;">')`
```{r dp-hinge-illustration, echo=FALSE, out.width=c(ow(170, 0.33), ow(170, 0.33)), fig.show="hold", fig.align=if (knitr::is_latex_output()) "center" else "default"}
dp_img(c(
...
`r h('<div style="display: grid; grid-template-columns: 1fr 400px; align-items: center; gap: 1.5em; border-left: 5px solid #C6402A; padding-left: 1em;">')`
`r h('<div style="min-width: 0;">')`
...
`r h('</div>')`
`r h('<div class="loss-fig" style="min-width: 0; width: 360px; text-align: right;">')`
```{r dp-pinball-illustration, echo=FALSE, out.width=ow(170), fig.align=if (knitr::is_latex_output()) "center" else "default"}
dp_img("work/02-drought_prediction/reports/figures/pinball.svg")
```
`r h('</div>')`
`r h('</div>')`
...
(ref:dp-naive-vs-film-cap) Dynamic global feature injection strategies. Naive injection is on the left and FiLM on the right. The FiLM depiction is by @perez2018.

```{r dp-naive-vs-film, echo=FALSE, fig.cap="(ref:dp-naive-vs-film-cap)", out.width=if (knitr::is_latex_output()) "60%" else "90%", fig.align="center", fig.pos="htbp"}
dp_img("work/02-drought_prediction/figures/architecture/Naive_vs_film.svg")
```

FiLM (feature-wise linear modulation) [@perez2018] instead conditions the network by applying a per-channel affine transform to the feature maps,
...
(ref:dp-convlstm-film-cap) Where FiLM sits inside the ConvLSTM cell: it conditions the embedded input feature map before the gate convolutions. The ConvLSTM cell depiction is by @marusov2024.

```{r dp-convlstm-film, echo=FALSE, fig.cap="(ref:dp-convlstm-film-cap)", out.width="75%", fig.align="center", fig.pos="htbp"}
dp_img("work/02-drought_prediction/figures/architecture/ConvLSTM_FiLM.svg")
```
...
```{r dp-naive-vs-static-encoder, echo=FALSE, fig.cap="Static spatial feature injection strategies. Naive injection is on the left and the learned encoder on the right (single and seasonal variants).", out.width=if (knitr::is_latex_output()) "60%" else "90%", fig.align="center", fig.pos="htbp"}
dp_img("work/02-drought_prediction/figures/architecture/Naive_vs_single_seasonal.svg")
```
...
The *single* variant uses one shared encoder, giving $[\,x_t^{\text{emb}}, E(S)\,]$, while the *seasonal* variant uses four encoders, one per meteorological season, selected by the season $s(m_t)$ of the target month, giving $[\,x_t^{\text{emb}}, E_{s(m_t)}(S)\,]$. Figure \@ref(fig:dp-convlstm-static) shows where the resulting encoder output enters the cell.

(ref:dp-convlstm-static-cap) Where the static encoder sits inside the ConvLSTM cell: it concatenates its output to the embedded input feature map before the gate convolutions. The ConvLSTM cell depiction is by @marusov2024.

```{r dp-convlstm-static, echo=FALSE, fig.cap="(ref:dp-convlstm-static-cap)", out.width="75%", fig.align="center", fig.pos="htbp"}
dp_img("work/02-drought_prediction/figures/architecture/ConvLSTM_static.svg")
```
...
Three reference forecasts are evaluated on the test period (2015--2024): persistence (the last observed SPEI carried forward twelve months), climatology (the training-period cell mean), and a per-cell linear trend extrapolated from the training years to the test years [@wilks2011; @jolliffe2003].
...
kbl(b, escape = FALSE, booktabs = TRUE, align = "llccc",
  ...
  kable_styling(latex_options = "hold_position", full_width = FALSE, font_size = if (knitr::is_latex_output()) 9 else 13)
```
...
kbl(d, escape = FALSE, booktabs = TRUE, align = c("l", rep("c", 9)),
  ...
  kable_styling(latex_options = c("hold_position", "scale_down"), font_size = if (knitr::is_latex_output()) 9 else 13)
```
...
```{r dp-film-stratified-main, echo=FALSE}
lc <- c("#3C3C3C", "#6EA5C8", "#286EA0", "#C6402A")
...
```{r dp-single-results, echo=FALSE}
lc <- c("#3C3C3C", "#6EA5C8", "#286EA0", "#C6402A")
grn <- "#005028"; red <- "#781414"
loss <- cell_spec(c("$\\mathrm{MSE}$", "$\\mathrm{hwMSE}_{w=1}$",
                    "$\\mathrm{hwMSE}_{w=5}$", "$\\mathrm{Pinball}_{\\tau=0.2}$"),
...
```{r dp-seasonal-results, echo=FALSE}
lc <- c("#3C3C3C", "#6EA5C8", "#286EA0", "#C6402A")
grn <- "#005028"; red <- "#781414"
loss <- cell_spec(c("$\\mathrm{MSE}$", "$\\mathrm{hwMSE}_{w=1}$",
                    "$\\mathrm{hwMSE}_{w=5}$", "$\\mathrm{Pinball}_{\\tau=0.2}$"),
...
```{r dp-frontier, echo=FALSE, fig.cap="RMSE against drought detection, as TPR (left) and F1 (right), for all 24 configurations and the persistence and climatology baselines. The vertical line marks the linear-trend RMSE.", out.width="48%", fig.show="hold", fig.align="center"}
dp_img(c(
...
Table: (\#tab:dp-spei-grades) SPEI drought classification [@vicenteserrano2010; @chen2025].

## Appendix: Feature importance {-}

Each input was occluded in turn (set to its training mean) and the frozen Pinball model re-scored on the test set [@zeiler2013].
Inputs are occluded rather than permuted because the driver fields are correlated [@hooker2021].
...
Table: (\#tab:dp-input-channels) Input channels per configuration, from a dynamic base of five gridded variables. Naive injection appends features as channels, while the learned encoders (static CNN, FiLM) route them into a dedicated pathway.
