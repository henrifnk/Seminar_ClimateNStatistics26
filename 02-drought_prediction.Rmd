...
SPEI values map onto standard drought categories (Table \@ref(tab:spei-grades)).
...
Table: (\#tab:spei-grades) SPEI drought classification [@vicente-serranoMultiscalarDroughtIndex2010; @chenSTATLSTMMultivariateSpatiotemporal2025].
...
The two feature groups can either be appended to the input or routed through a learned pathway, which changes how many channels the ConvLSTM input $x$ carries (Table \@ref(tab:input-channels)).
...
Table: (\#tab:input-channels) Input channels per configuration, from a dynamic base of five gridded variables. Naive injection appends features as channels, while the learned encoders (static CNN, FiLM) route them into a dedicated pathway.
