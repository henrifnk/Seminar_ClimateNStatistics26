...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Creative Commons License}]{book_files/by-nc-sa.png}}
...
{\centering \includegraphics[width=1\linewidth]{work/02-drought_prediction/figures/general/drought_types_vector} 
...
{\centering \includegraphics[width=0.7\linewidth]{work/02-drought_prediction/figures/raw/topography} 
...
{\centering \includegraphics[width=1\linewidth]{work/02-drought_prediction/figures/raw/al/spei} 
...
{\centering \includegraphics[width=1\linewidth]{work/02-drought_prediction/figures/raw/slides/mediterranean_slt} 
...
{\centering \includegraphics[width=1\linewidth]{work/02-drought_prediction/figures/processed/exploratory/drought_frequency_severe} 
...
{\centering \includegraphics[width=0.75\linewidth]{work/02-drought_prediction/figures/architecture/ConvLSTM} 
...
\begin{center}\includegraphics[width=0.33\linewidth]{work/02-drought_prediction/reports/figures/mse} \end{center}
...
\begin{center}\includegraphics[width=0.33\linewidth]{work/02-drought_prediction/reports/figures/hinge_weight} \includegraphics[width=0.33\linewidth]{work/02-drought_prediction/reports/figures/hinge_loss} \end{center}
...
\begin{center}\includegraphics[width=0.33\linewidth]{work/02-drought_prediction/reports/figures/pinball} \end{center}
...
{\centering \includegraphics[width=0.6\linewidth]{work/02-drought_prediction/figures/architecture/Naive_vs_film} 
...
{\centering \includegraphics[width=0.75\linewidth]{work/02-drought_prediction/figures/architecture/ConvLSTM_FiLM} 
...
{\centering \includegraphics[width=0.6\linewidth]{work/02-drought_prediction/figures/architecture/Naive_vs_single_seasonal} 
...
{\centering \includegraphics[width=0.75\linewidth]{work/02-drought_prediction/figures/architecture/ConvLSTM_static} 
...
{\centering \includegraphics[width=0.48\linewidth]{work/02-drought_prediction/figures/frontier_tpr_light} \includegraphics[width=0.48\linewidth]{work/02-drought_prediction/figures/frontier_f1_light} 
...
{\centering \includegraphics[width=0.95\linewidth]{work/02-drought_prediction/figures/interpretability/feature_importance_across_conditions} 
...
\chapter{Climate downscaling using diffusion models}\label{ddm}

\emph{Author: Alexander Winterstetter}
...
climate models are typically run at resolutions too coarse to represent the local-scale processes on which adaptation and risk-management decisions depend \citep{ling2024, merizzi2024}.

Downscaling is performed either dynamically, utilizing regional climate models, or statistically, by learning a relationship between coarse and fine climate fields \citep{ling2024}.
...
and super-resolution of natural images \citep{addison2022, merizzi2024}. In particular, the use of diffusion models is an active area of research \citep{springenberg2026}.
...
Downscaling thereby becomes a probabilistic task, in which the ensemble spread quantifies the uncertainty of the coarse-to-fine mapping \citep{watt2024, springenberg2026}.

Implementations differ mainly in what the diffusion model produces. Some studies generate the residual between the interpolated coarse field and the high-resolution field \citep{watt2024, lopezgomez2025},
and add it back to the interpolated field. Alternatively, the residual is taken with respect to a separate regression model \citep{mardani2025, tomasi2025},
or the model outputs the full field directly using the coarse field as context alone \citep{addison2026, merizzi2024}.
...
recovering the original resolution \citep{merizzi2024, watt2024}. A smaller number of studies utilize the trained model to generate climate projections and assess how well the climate change signal is preserved in them \citep{addison2026, aich2026}.
...
For \textbf{calibration assessment}, it evaluates whether the \citet{watt2024} architecture generates accurate and calibrated ensembles over the complex topography of the Alps, downscaling reanalysis fields from 12 km to a target resolution of 3.4 km.
Then for \textbf{signal preservation}, the trained model is assessed on how well it preserves the climate change signal when applied to future climate projections.

\section{Diffusion models for image generation}\label{ddm-theory}
...
e.g., a coarse version of the target image. The underlying diffusion-model framework was established by \citet{song2021} and refined by \citet{karras2022} into the EDM-formulation, which is used in several downscaling papers
\citep{watt2024, mardani2025, lopezgomez2025} and is adopted in this work.

\subsection{Training the model}\label{training-the-model}

Conceptually, the idea of diffusion models is to generate a high-quality image by iteratively denoising a Gaussian noise sample \citep{song2021}. During training, each clean target field \(\boldsymbol{x}_0\) is corrupted with Gaussian noise \(\boldsymbol{\epsilon}\)
at a noise level \(\sigma\) randomly drawn from a log-normal distribution \citep{karras2022}:
...
The model \(D_\theta(\boldsymbol{x}_\sigma;\, \sigma,\, \boldsymbol{y})\) is a U-Net \citep{ronneberger2015}, trained to estimate the noise-free image from a corrupted image, denoted as
...
the noise-level distribution, and the injected noise, where \(\lambda(\sigma)\) weights the contribution of each noise level so that all levels contribute comparably to the loss \citep{karras2022}.

\subsection{Generation of samples}\label{generation-of-samples}
...
weighted by the ratio of the next to the current noise level. Simplified, it can be expressed as \citep{karras2022}:
...
and applies a second-order correction to the update, both omitted above for readability \citep{karras2022}.
An extensive visualization of the generation process can be found in \hyperref[ddm-appendix-a]{Appendix A}.
...
Temperature varies smoothly and its fine-scale structure is strongly tied to terrain elevation \citep{tomasi2025}. Precipitation is intermittent, non-negative and right-skewed.
Additionally, the coarse-scale state does not uniquely determine the fine-scale field \citep{addison2026}.
...
\subsection{Dataset information}\label{ddm-dataset-information}

The dataset consists of reanalysis and climate projection data and was provided by the \href{https://www.alpine-space.eu/project/a-drop/}{Alpine DROught Prediction (A-DROP)} project covering the Alpine domain.
\textbf{Reanalysis data} is a gridded reconstruction of past weather states and is provided in this work at a resolution of 500 m (1749 × 2198 pixels over the Alpine domain), downscaled from the native CERRA reanalysis.
...
\textbf{Projection data} encompasses a simulated trajectory of how Alpine climate might develop over the current century. This work assigns the years to specific Global Warming Levels (GWL) as outlined in Table \ref{tab:ddm-gwl-periods}.
...
The data was bias-corrected against the reanalysis data utilizing the MBCn-algorithm \citep{cannon2018} and is only available at a resolution of 12 km (77 × 91 pixels), considerably coarser than the reanalysis data.
...
The distributional shift between the reanalysis data and the four projection periods is visualized in Figure \ref{fig:ddm-distribution-shift}.
A more detailed breakdown of the shift can be found in \hyperref[ddm-appendix-b]{Appendix B}.

\begin{table}[!h]
\centering
\caption{\label{tab:ddm-gwl-periods}Global warming levels for the projection assessment, defined as non-overlapping 20-year windows approximately following IPCC AR6 WGI Ch. 4. Mean warming is relative to the pre-industrial baseline (1850–1900).}
\centering
\begin{tabular}[t]{ccc}
\toprule
\cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{GWL period}}} & \cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{Mean warming in period (°C)}}} & \cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{Years}}}\\
\midrule
...
{\centering \includegraphics[width=1\linewidth]{work/13-diffusion_downscaling/figures/projection/ddm_3_distribution_periods} 

}

\caption{Domain-wide distributions of daily temperature and precipitation, for the reanalysis data and the four projection periods.}\label{fig:ddm-distribution-shift}
\end{figure}
...
\subsection{Modeling setup}\label{ddm-modeling-setup}

The diffusion model technique is applied to the climate downscaling context following \citet{watt2024}.
The pipeline is outlined in Figure \ref{fig:ddm-downscaling-workflow}. In a first step, the original coarse 12 km climate field is linearly interpolated to a smoothed version at the target resolution.
...
{\centering \includegraphics[width=1\linewidth]{work/13-diffusion_downscaling/figures/training/ddm_model_structure} 

}

\caption{Residual downscaling workflow. The coarse field is linearly interpolated, the diffusion model generates the residual, and their sum yields the downscaled field.}\label{fig:ddm-downscaling-workflow}
\end{figure}

\subsection{Training and inference setup}\label{ddm-training-inference-setup}

The training procedure follows established approaches for diffusion-based downscaling \citep{watt2024, addison2026}.
...
Whether the diffusion model is also able to handle the temporal shift induced by climate change is assessed in Section \ref{ddm-signal-preservation}.

The model was trained for a fixed budget of 20 epochs, by which point the validation loss had largely flattened.
Further information on the compute time can be found in \hyperref[ddm-appendix-e]{Appendix E}.
...
\subsection{Evaluation metrics}\label{ddm-evaluation-metrics}

The accuracy of a generated ensemble with regard to an observation can be measured using the continuous ranked probability score (CRPS), rewarding both accuracy of the individual members and appropriate spread between them \citep{gneiting2007}.
...
The first term penalizes the distance of the forecast from the observation and the second rewards spread \citep{gneiting2007}.
An unbiased estimator of the CRPS was applied to account for the bias introduced by the finite ensemble, following practice from other works \citep{mardani2025, addison2026}.
...
Related works assess the amount of spatial variability in a field using the radially averaged power spectral density (RAPSD), which quantifies the amount of variability a field contains at each spatial scale \citep{addison2026, tomasi2025}.
However, the RAPSD captures only the amount of the variability, not whether it is placed at the correct locations.

Following \citet{mardani2025} and \citet{addison2026}, calibration is assessed using the spread--skill relationship, with the ensemble spread corrected for the finite ensemble size. In a calibrated ensemble, the spread of the ensemble members matches the error of the ensemble mean on average. A spread--skill ratio below one indicates an underdispersed ensemble,
whose members vary less than their actual error would justify.

\section{Results}\label{results-1}

\subsection{Calibration assessment}\label{ddm-calibration-assessment}

Table \ref{tab:ddm-crps-skill} shows the MAE for the baseline interpolation model and the CRPS for the diffusion model for temperature and precipitation,
...
The spatial location of the error between the ensemble mean and the ground truth is visualized in Figure \ref{fig:ddm-error-prone-areas}, which indicates that the errors are concentrated along the Alpine ridge.
\hyperref[ddm-appendix-c]{Appendix C} gives further information on where the model applies the largest correction to the interpolated field.
Figure \ref{fig:ddm-power-spectrum} assesses how well the generated fields capture the spatial variability of the ground truth beyond the evaluation metrics.
...
\begin{table}[!h]
\centering
\caption{\label{tab:ddm-crps-skill}Comparison of ensemble CRPS to the interpolation baseline MAE for temperature and precipitation.}
\centering
\begin{tabular}[t]{llcc}
\toprule
\cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{Metric}}} & \cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{Model}}} & \cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{Temperature (°C)}}} & \cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{Precipitation (kg/m²)}}}\\
\midrule
...
{\centering \includegraphics[width=1\linewidth]{work/13-diffusion_downscaling/figures/calibration/ddm_3_mae_spatial} 

}

\caption{Mean absolute error of the ensemble mean over the Alpine domain, in °C and kg/m².}\label{fig:ddm-error-prone-areas}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/13-diffusion_downscaling/figures/calibration/ddm_1_power_spectrum} 

}

\caption{Radially averaged power spectral density for temperature and precipitation}\label{fig:ddm-power-spectrum}
\end{figure}

Figure \ref{fig:ddm-spread-skill} addresses the calibration of the ensemble by grouping the pixels into 30 equal-count bins by ensemble spread and calculating the RMSE of the ensemble mean within each bin.
...
{\centering \includegraphics[width=1\linewidth]{work/13-diffusion_downscaling/figures/calibration/ddm_2_spread_skill} 

}

\caption{Spread--skill relationship of the diffusion ensemble. A calibrated ensemble follows the one-to-one line.}\label{fig:ddm-spread-skill}
\end{figure}

\subsection{Signal preservation}\label{ddm-signal-preservation}
...
Figure \ref{fig:ddm-proj-period-means} compares the average temperature and precipitation mass between the interpolated climate field and the downscaled climate field.
...
The surplus may follow from the fine-scale variance inflation reported in Section \ref{ddm-calibration-assessment}.

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/13-diffusion_downscaling/figures/projection/ddm_2_period_means} 

}

\caption{Effect of the downscaling step per global warming level, in percent relative to the interpolated field.}\label{fig:ddm-proj-period-means}
\end{figure}

Figure \ref{fig:ddm-proj-delta-gwl4} provides a spatial assessment of how the climate change signal is preserved for GWL4, the period containing the largest distributional shift from the distribution of the reanalysis dataset.
...
{\centering \includegraphics[width=1\linewidth]{work/13-diffusion_downscaling/figures/projection/ddm_1_delta/ddm_gwl4} 

}

\caption{Spatial climate change signal at GWL4 relative to PRESENT. Rows: temperature, total precipitation mass. Columns: interpolated field, downscaled ensemble mean, their difference.}\label{fig:ddm-proj-delta-gwl4}
\end{figure}

Figures \ref{fig:ddm-max-temp-day} and \ref{fig:ddm-max-precip-day} present the day with the highest domain-mean temperature and the day with the highest precipitation mass
in the projection data. The corresponding numbers on how the statistics of the field are affected by the models are given in Table \ref{tab:ddm-max-day-stats}.
...
This may be due to the inflated fine-scale variance reported in Section \ref{ddm-calibration-assessment}, which is also evident in the granular speckle that neither the coarse nor the interpolated field shows.

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/13-diffusion_downscaling/figures/projection/ddm_6_highest_mean_temperature_day_2080-08-13} 

}

\caption{Day with the highest domain-mean temperature. Left to right: native 12 km grid, interpolation, downscaled ensemble mean.}\label{fig:ddm-max-temp-day}
\end{figure}
...
{\centering \includegraphics[width=1\linewidth]{work/13-diffusion_downscaling/figures/projection/ddm_6_highest_precipitation_mass_day_2056-09-10} 

}

\caption{Day with the highest precipitation mass. Panels as in Figure \ref{fig:ddm-max-temp-day}.}\label{fig:ddm-max-precip-day}
\end{figure}

\begin{table}[!h]
\centering
\caption{\label{tab:ddm-max-day-stats}Field statistics for the hottest day (2080-08-13, temperature in °C) and the wettest day (2056-09-10, precipitation in mm/day). Interpolation is the effect of the linear interpolation (interpolated minus native), Model the diffusion residual (downscaled minus interpolated).}
\centering
\begin{tabular}[t]{>{}lrrrrr}
\toprule
\multicolumn{1}{c}{\cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{ }}}} & \multicolumn{3}{c}{\cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{Field value}}}} & \multicolumn{2}{c}{\cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{Effect}}}} \\
\cmidrule(l{3pt}r{3pt}){2-4} \cmidrule(l{3pt}r{3pt}){5-6}
\cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{}}} & \cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{Native (12 km)}}} & \cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{Interpolated}}} & \cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{Downscaled}}} & \cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{Interpolation}}} & \cellcolor[HTML]{3f3f3f}{\textcolor{white}{\textbf{Model}}}\\
\midrule
...
A short analysis of whether the spread of the generated ensembles is affected by the distributional shift can be found in \hyperref[ddm-appendix-d]{Appendix D}.

\section{Discussion}\label{ddm-discussion}

The diagnostic findings line up with previous work.
\citet{mardani2025} note that temperature downscaling is largely driven by sub-grid topography, which is learnable deterministically from static inputs.
...
\citet{mardani2025} and \citet{lopezgomez2025} both report underdispersed ensembles, while other work focusing on generating the full field instead of only the residual field reports well-calibrated spread \citep{aich2026, addison2026}.

The projection results are consistent with \citet{aich2026}, who find the change signal preserved under a high-emission scenario. This work shows that the downscaling step does not distort the signal it is given,
...
\citet{addison2026} have projection data at both resolutions and can therefore compare their downscaled change signal against a fine-scale reference.
...
The orography field was only available at 5.5 km, which may explain the error concentration along the Alpine ridge reported in Section \ref{ddm-calibration-assessment}.
...
Several directions follow from this work. Multiple options exist to improve the model's capability in downscaling precipitation. One is to transform the variable before training, for example with a log or square-root mapping, which reduces its skew \citep{addison2026, aich2026}.
\citet{lopezgomez2025} gave the model additional context, e.g.~information on surface pressure, for downscaling precipitation.
Another is to change the architecture to the regression-residual approach, so that a conditional regression first brings the coarse field to the target resolution and the diffusion model only has to handle the stochastic component \citep{mardani2025, tomasi2025} or to have the model generate the full climate field \citep{addison2026, aich2026}.
...
Checkpoint selection could also be introduced, based on validation loss as well as accuracy of the generated ensemble on the validation set \citep{addison2026}.
Finally, a feature importance analysis on the contextual information could show how much the orography, the date and the coarse input field each contribute \citep{lopezgomez2025}.

\subsection{Note on the dataset}\label{note-on-the-dataset}
...
Further information on the A-DROP project can be found at the following link: \url{https://www.alpine-space.eu/project/a-drop/}

\section{Appendix}\label{appendix}

\subsection{Appendix A - More information on reverse diffusion process}\label{ddm-appendix-a}
...
Figures \ref{fig:ddm-filmstrip-tas} and \ref{fig:ddm-filmstrip-pr} show this process for the temperature and the precipitation residual field.

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/13-diffusion_downscaling/figures/diagnostics/denoiser_estimate_2016-06-23_seed42/ddm_filmstrip_tas_residual} 

}

\caption{Reverse diffusion process for the temperature residual field.}\label{fig:ddm-filmstrip-tas}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/13-diffusion_downscaling/figures/diagnostics/denoiser_estimate_2016-06-23_seed42/ddm_filmstrip_pr_residual} 

}

\caption{Reverse diffusion process for the precipitation residual field.}\label{fig:ddm-filmstrip-pr}
\end{figure}

\subsection{Appendix B - Distributional shift of the climate variables across global warming levels}\label{ddm-appendix-b}

The ridgeline visualization of Figure \ref{fig:ddm-distribution-shift} is supplemented by the percentile plot in Figure \ref{fig:ddm-quantile-delta}, which shows how far each percentile of the
...
{\centering \includegraphics[width=1\linewidth]{work/13-diffusion_downscaling/figures/projection/ddm_4_quantile_delta} 

}

\caption{Distribution shift relative to PRESENT, by percentile and global warming level. The dashed line marks no change.}\label{fig:ddm-quantile-delta}
\end{figure}

\subsection{Appendix C - Mean downscaling correction}\label{ddm-appendix-c}

Figure \ref{fig:ddm-downscaling-correction} shows the correction the diffusion model applies to the interpolated field, averaged over all evaluated days.
...
{\centering \includegraphics[width=1\linewidth]{work/13-diffusion_downscaling/figures/calibration/ddm_4_downscaling_correction} 

}

\caption{Mean correction added by the diffusion model to the interpolated field, averaged over the evaluated days of the reanalysis test set. Left: temperature, right: precipitation.}\label{fig:ddm-downscaling-correction}
\end{figure}

\subsection{Appendix D - Ensemble spread over projection period}\label{ddm-appendix-d}

Figure \ref{fig:ddm-spread-persistence} shows the distribution of the daily ensemble spread of the four-member ensemble over the evaluated days of each global warming level.
...
{\centering \includegraphics[width=1\linewidth]{work/13-diffusion_downscaling/figures/projection/ddm_5_spread_persistence} 

}

\caption{Daily ensemble spread of the projection ensembles, per global warming level. Each row is the histogram over the evaluated days of one period.}\label{fig:ddm-spread-persistence}
\end{figure}

\subsection{Appendix E - Compute time}\label{ddm-appendix-e}
...
\section{Usage of AI tools}\label{usage-of-ai-tools}
...
{\centering \includegraphics[width=0.85\linewidth]{work/03-hydrology_cc/figures/regions_of_swiss_map} 
...
{\centering \includegraphics[width=0.9\linewidth]{work/03-hydrology_cc/figures/stl_example_overlay} 
...
{\centering \includegraphics[width=0.9\linewidth]{work/03-hydrology_cc/figures/stl_example_snr} 
...
{\centering \includegraphics[width=0.9\linewidth]{work/03-hydrology_cc/figures/emergence_year_uncertainty} 
...
{\centering \includegraphics[width=0.95\linewidth]{work/03-hydrology_cc/figures/emergence_extent_by_region} 
...
{\centering \includegraphics[width=0.95\linewidth]{work/03-hydrology_cc/figures/seasonal_emergence_extent_curves} 
...
{\centering \includegraphics[width=0.9\linewidth]{work/03-hydrology_cc/figures/seasonal_direction_bars} 
...
{\centering \includegraphics[width=0.95\linewidth]{work/03-hydrology_cc/figures/seasonal_direction_by_region} 
...
{\centering \includegraphics[width=0.95\linewidth]{work/03-hydrology_cc/figures/seasonal_emergence_heatmap} 
...
\begin{center}\includegraphics[width=\linewidth]{book_files/figure-latex/nh-transfer-by-group-nse-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{book_files/figure-latex/nh-transfer-delta-bars-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{book_files/figure-latex/nh-transfer-basin-box-ecdf-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{book_files/figure-latex/nh-transfer-basin-ecdf-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{book_files/figure-latex/nh-transfer-map-by-group-book-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{book_files/figure-latex/nh-transfer-coverage-bars-book-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{book_files/figure-latex/nh-transfer-group-size-dist-book-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{book_files/figure-latex/nh-transfer-overall-nse-bars-book-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{book_files/figure-latex/nh-transfer-overall-kge-bars-book-1} \end{center}

Consistent ordering under KGE supports the same conclusion as NSE and strengthens
cross-metric robustness.

\begin{center}\includegraphics[width=\linewidth]{book_files/figure-latex/nh-transfer-delta-kge-bars-book-1} \end{center}

Group-level KGE deltas show whether transfer gains persist once correlation,
variability, and bias are considered jointly.

\begin{center}\includegraphics[width=\linewidth]{book_files/figure-latex/nh-transfer-local-ft-scatter-book-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{book_files/figure-latex/nh-transfer-local-vs-gain-book-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{book_files/figure-latex/nh-transfer-arch-nse-book-1} \end{center}
...
\begin{center}\includegraphics[width=\linewidth]{book_files/figure-latex/nh-transfer-group05-bars-book-1} \end{center}
...
{\centering \includegraphics[width=0.8\linewidth]{work/05-riverine_heat/figures/05-stations_map} 
...
{\centering \includegraphics[width=0.7\linewidth]{work/05-riverine_heat/figures/05-threshold_kemmern} 
...
{\centering \includegraphics[width=0.8\linewidth]{work/05-riverine_heat/figures/05-heatwave_events_overview} 
...
{\centering \includegraphics[width=0.8\linewidth]{work/05-riverine_heat/figures/05-heatwave_events_plot} 
...
{\centering \includegraphics[width=0.8\linewidth]{work/05-riverine_heat/figures/05-mean_duration_overview} 
...
{\centering \includegraphics[width=0.8\linewidth]{work/05-riverine_heat/figures/05-duration_plot} 
...
{\centering \includegraphics[width=0.8\linewidth]{work/05-riverine_heat/figures/05-mean_intensity_overview} 
...
{\centering \includegraphics[width=0.8\linewidth]{work/05-riverine_heat/figures/05-intensity_plot} 
...
{\centering \includegraphics[width=0.8\linewidth]{work/05-riverine_heat/figures/05-polygon_temporal} 
...
{\centering \includegraphics[width=\linewidth]{work/05-riverine_heat/figures/05-polygon_spatial} 
...
\emph{Author: Alexander Zielke Esteban}
...
The data used in this study originate from the \href{https://henrifnk.github.io/henrifnk/projects.html}{BayWat-AI} project and are provided by the Bavarian Landesamt für Umwelt (LfU) and the Gewässerkundlicher Dienst Bayern (GKD).

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/06-interpretable_heat/figures/images/stations_map} 

}

\caption{Map of the 14 measurement stations along the river Main and its tributaries Regnitz, Fränkische Saale, Itz and Wern. Map created with Felt and River Data from Natural Earth Data.}\label{fig:irh-fig-stations-map}
\end{figure}
...
\textbf{Static variables (station-specific)}
\begin{itemize}
  \item Elevation (m a.s.l.)
  \item Slope (\textdegree)
  \item Forest cover (\%)
  \item Impervious surface (\%) within a radius of:
  \begin{itemize}
    \item 500 m
    \item 1,000 m
    \item 2,000 m
  ...
  \begin{itemize}
    ...
    \item River source
  \end{itemize}
  \item Total river length (km)
\end{itemize}
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
\textbf{Dynamic variables}
\begin{itemize}
  \item Water temperature (\textdegree C)
  \item Air temperature (\textdegree C)
  \item Precipitation (mm)
  \item Wind speed (m/s)
  \item Relative humidity [0-1]
  \item Solar radiation (Wh/$\text{m}^2$)
  \item Discharge ($\text{m}^3$/s)
\end{itemize}
\end{minipage}
...
Figure \ref{fig:irh-fig-stations-missing-patterns} shows the resulting missing-data patterns of the dynamic variables.

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/06-interpretable_heat/figures/stations_missings} 

}

\caption{ Missing Patterns of dynamic variables on a daily basis for each station. A daily value is missing if at least one of the eight three-hourly observations for the respective variable and station is missing.}\label{fig:irh-fig-stations-missing-patterns}
\end{figure}
...
\textbf{Long-Short-Term-Memory (LSTM) Model}

A Long-Short-Term Memory (LSTM) network is a recurrent neural network (RNN) architecture designed for sequential data, such as time series \citep{zhang2023}. LSTMs mitigate the vanishing-gradient problem of conventional RNNs by incorporating a separate cell state with gated information flow. This allows information to be retained or discarded over longer time intervals, enabling the model to capture temporal dependencies.

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/06-interpretable_heat/figures/images/lstm_sequential_processing} 

}

\caption{Sequential processing of the input data by an LSTM. Each cell represents one time step and receives the current input as well as the hidden and cell states from the previous time step.}\label{fig:irh-fig-lstm-seq-proc}
\end{figure}

The data are processed sequentially, one time step after another, as illustrated in Figure \ref{fig:irh-fig-lstm-seq-proc} (in this study, one time step corresponds to one day). At each step, an LSTM cell receives the current input together with the hidden and cell state from the previous time step:
...
{\centering \includegraphics[width=1\linewidth]{work/06-interpretable_heat/figures/images/lstm_cell_structure} 

}

\caption{Structure of an LSTM cell. The current input and previous hidden state are combined and passed through the forget, input and output gates and the input node to update the cell and hidden states.}\label{fig:irh-fig-lstm-cell-structure}
\end{figure}

Figure \ref{fig:irh-fig-lstm-cell-structure} illustrates the computations within an LSTM cell.
...
\subsection{Implementation}\label{sec-model-impl}

\textbf{Prediction Setting}
...
The model was implemented in Python using the Neural Hydrology library. The main model configuration is summarized in Table \ref{tab:lstm-config}.

\begin{longtable}[]{@{}ll@{}}
\caption{\label{tab:lstm-config} Configuration of the LSTM model}\tabularnewline
\toprule\noalign{}
Parameter & Value \\
\midrule\noalign{}
\endfirsthead
...
\textbf{Input and Output Structure}
...
where the dimensions correspond to observations, variables and time lags, respectively, as illustrated in Figure \ref{fig:irh-fig-lstm-input-output-structure}. The entry at position \((i, x, l)\) contains the value of feature \(x\) for the \(i\)-th observation at lag \(l\).

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/06-interpretable_heat/figures/images/lstm_input_output_structure} 

}

\caption{Input and output structure of the LSTM model. The dynamic features are arranged in a three-dimensional tensor with dimensions corresponding to samples, variables and lags. The static features are replicated across all time steps and concatenated with the dynamic features. The model produces a prediction for each time step, resulting in a $(n_{samples}, 1, n_{lags})$ tensor, of which only the prediction at lag 0 is used as the target prediction.}\label{fig:irh-fig-lstm-input-output-structure}
\end{figure}
...
First, we used \textbf{Permutation Feature Importance} to assess the overall importance of the input features. Based on these results, we examined the two most important features from four complementary perspectives:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Overall dependence over the whole last year:}
...
\item
  \textbf{Dependence on the most recent observation:}
...
\item
  \textbf{Lag Importances of a feature:}
...
\item
  \textbf{Interaction between the two most important features:}
...
\subsection{Permutation Feature Importance}\label{permutation-feature-importance}
...
The permutation itself differs between the two feature types, as illustrated in Figure \ref{fig:irh-fig-tensor-feature-perm}. For a static feature \(S_j\), the observation \(s_j^{(1)}, \dots, s_j^{(n)}\) are permuted randomly across observations. For a dynamic feature \(X_j\), the time series \(x_j^{(i, 0)}, \dots, x_j^{(i, 364)}\) is permuted within each observation \(i\).

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{work/06-interpretable_heat/figures/images/tensor_feature_permutation} 

}

\caption{Manipulation of the static feature matrix and dynamic feature tensor for the calculation of Permutation Feature Importance. For static features, the corresponding feature values are permuted across all observations. For dynamic features, the time series of the corresponding feature is permuted within each observation.}\label{fig:irh-fig-tensor-feature-perm}
\end{figure}
...
\subsection{Partial Dependence Plots (for dynamic features)}\label{partial-dependence-plots-for-dynamic-features}
...
To this end, we manipulate the entire time series of the feature at once (leaving all other features untouched), distinguishing between interval-scaled and ratio-scaled features, as shown in figure \ref{fig:irh-fig-tensor-pdp}:
...
{\centering \includegraphics[width=1\linewidth]{work/06-interpretable_heat/figures/images/tensor_pdp} 

}

\caption{Manipulation of the dynamic feature tensor for the computation of the Partial Dependence Plot. For interval-scaled features, a constant offset a is added to the corresponding feature (left tensor); for ratio-scaled features, the corresponding feature is multiplied by a constant factor a (right tensor).}\label{fig:irh-fig-tensor-pdp}
\end{figure}
...
\textbf{Limitation}: For correlated features, manipulating one feature while leaving all others unchanged can create unrealistic, out of distribution feature combinations, which may distort the interpretation of the resulting PDP curve.
...
\textbf{Notation:} Let the range of \(X_j\) at lag \(l\) be divided into \(m\) bins \(N_{j, l}(1), \dots, N_{j, l}(m)\) with bin bounds \(z_{j, l}(0) < z_{j, l}(1) < \dots < z_{j, l}(m)\) and let
...
\item
  ...
  \]
\item
  Accumulate the local effects of all bins up to \(a\):
  \[
...
\subsection{Lag Importance using ALE}\label{lag-importance-using-ale}
...
\subsection{2D Partial Dependence Plot}\label{d-partial-dependence-plot}
...
\textbf{Limitation}: As with the one-dimensional PDP, this approach can produce unrealistic, out-of-distribution feature combinations if \(X_j\) and \(X_k\) are strongly correlated with each other or with other features.
...
\subsection{Permutation Feature Importance}\label{permutation-feature-importance-1}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/06-interpretable_heat/figures/int_perm_importance} 

}

\caption{Permutation Feature Importance of static (orange) and dynamic (blue) features.}\label{fig:irh-fig-feat-imp}
\end{figure}

As shown in Figure \ref{fig:irh-fig-feat-imp}, the most important features according to Permutation Feature Importance are Air Temperature and Solar Radiation. Their importance values are more than ten times higher than the highest importance value among the static features.

\newpage

\subsection{Partial Dependence Plots (PDP)}\label{partial-dependence-plots-pdp}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/06-interpretable_heat/figures/int_pdp} 

}

\caption{Partial Dependence Plots for Air Temperature and Solar Radiation. Each gray line represents the ICE curve of an individual observation, that is, how that observation's prediction would change under the corresponding offset or factor, while the red line shows the averaged PDP curve.}\label{fig:irh-fig-pdp}
\end{figure}

The PDP for Air Temperature (\ref{fig:irh-fig-pdp}) shows a nearly linear trend for offsets with small absolute value, flattening for offsets with large absolute value. This is consistent with our expectations: at sufficient high or low air temperatures, we would expect the effect on water temperature to be damped relative to the (nearly) linear response observed for smaller offsets.
...
\subsection{ALE Curves for the Most Recent Day (Lag 0)}\label{ale-curves-for-the-most-recent-day-lag-0}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/06-interpretable_heat/figures/int_ale} 

}

\caption{ALE curves once across all observations (black) and separately for observations falling within each season. The points represent the bin bounds, connected by lines that trace the estimated local effect across the feature range. The rug plot below each curve shows the feature's distribution within each season.}\label{fig:irh-fig-ale}
\end{figure}
...
\subsection{Lag Importance (ALE)}\label{lag-importance-ale}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/06-interpretable_heat/figures/int_lag_importance} 

}

\caption{Lag Importance for different Lags of Air Temperature and Solar Radiation. Note that the y-axes of the two plots are scaled differently.}\label{fig:irh-fig-lag-importance}
\end{figure}
...
\subsection{2D Partial Dependence Plot}\label{d-partial-dependence-plot-1}

\begin{figure}

{\centering \includegraphics[width=0.8\linewidth]{work/06-interpretable_heat/figures/int_pdp2d_Ta_C_rad_whm2} 

}

\caption{Two-dimensional Partial Dependence Plot of Air Temperature and Solar Radiation}\label{fig:irh-fig-pdp-2d}
\end{figure}
...
\section{Appendix: AI Declaration}\label{appendix-ai-declaration-1}
...
{\centering \includegraphics[width=1\linewidth]{work/01-climate_uncertainty/figures/fig01_study_region_map} 
...
{\centering \includegraphics[width=1\linewidth]{work/01-climate_uncertainty/figures/workflow} 
...
{\centering \includegraphics[width=1\linewidth]{work/01-climate_uncertainty/figures/fig02_regional_jja_warming_timeseries} 
...
{\centering \includegraphics[width=1\linewidth]{work/01-climate_uncertainty/figures/fig03_model_forced_response} 
...
{\centering \includegraphics[width=0.65\linewidth]{work/01-climate_uncertainty/figures/fig04_internal_variability} 
...
{\centering \includegraphics[width=0.65\linewidth]{work/01-climate_uncertainty/figures/fig05_model_uncertainty} 
...
{\centering \includegraphics[width=0.8\linewidth]{work/01-climate_uncertainty/figures/fig06_uncertainty_decomposition} 
...
{\centering \includegraphics[width=0.5\linewidth]{work/01-climate_uncertainty/figures/fig07_total_uncertainty_heatmap} 
...
{\centering \includegraphics[width=1\linewidth]{work/01-climate_uncertainty/figures/fig08_warming_distribution_prediction_ranges} 
...
{\centering \includegraphics[width=0.65\linewidth]{work/01-climate_uncertainty/figures/fig09_total_snr} 
...
{\centering \includegraphics[width=0.65\linewidth]{work/01-climate_uncertainty/figures/fig10_internal_snr} 
...
{\centering \includegraphics[width=1\linewidth]{work/01-climate_uncertainty/figures/fig11_threshold_exceedance_probability} 
...
{\centering \includegraphics[width=1\linewidth]{work/01-climate_uncertainty/figures/fig15b_mid_century_warming_distribution_thresholds} 
...
{\centering \includegraphics[width=1\linewidth]{work/01-climate_uncertainty/figures/fig16_bootstrap_ci_uncertainty_metrics} 
...
{\centering \includegraphics[width=1\linewidth]{work/01-climate_uncertainty/figures/fig17_bootstrap_ci_snr_threshold_probability} 
...
{\centering \includegraphics[width=1\linewidth]{work/01-climate_uncertainty/figures/fig18_balanced_20_member_sensitivity} 
...
{\centering \includegraphics[width=0.7\linewidth]{work/01-climate_uncertainty/figures/fig12_regional_temporal_internal_sd} 
...
{\centering \includegraphics[width=0.7\linewidth]{work/01-climate_uncertainty/figures/fig13_regional_temporal_total_sd} 
...
{\centering \includegraphics[width=1\linewidth]{work/01-climate_uncertainty/figures/fig15a_threshold_centered_snr_exceedance_probability} 
...
\chapter{Deep Learning for Change-Point Detection in Climate Time Series}\label{cs}

\emph{Author: Maximilian Bur}
...
Climate change is often summarized as a gradual long-term warming trend. Many climate-related processes, however, also exhibit abrupt or accelerated structural changes, including shifts in variability, trend slope, or temporal dependence. Change-point detection (CPD) provides a statistical framework for asking whether and when such transitions occur \citep{killick2012, truong2020}. In climate statistics, the problem is particularly demanding: series are non-stationary and autocorrelated, early instrumental records can be noisy or sparsely observed, and verified labels for historical regime shifts are typically unavailable.

Classical CPD methods rely on an explicit cost function or likelihood and yield interpretable segmentations under clear model assumptions. Their strength is also their limitation: the practitioner must specify which property is expected to change. Deep-learning approaches take a complementary route. Rather than monitoring a single preselected statistic, they learn feature representations from the data and infer potential changes from those representations \citep{xu2025}. This is attractive when several statistical properties may change jointly or when a suitable parametric cost is difficult to specify in advance.

Deep CPD should not be misunderstood as a climate-specific methodology in its own right. As emphasized by \citep{xu2025}, much of the literature concerns representation learning for sequential data, whereas only a smaller part is CPD-specific postprocessing that converts continuous scores into discrete change times. Statistical analysis remains necessary to assess whether a detected pattern is credible and scientifically meaningful.

The primary objective of this report is to summarize and illustrate the framework for deep-learning-based CPD reviewed by \citep{xu2025}, rather than to develop or comprehensively benchmark a new detection algorithm. To make the reviewed concepts concrete, the report implements a self-supervised pipeline for a univariate climate series, consisting of wavelet preprocessing, a convolutional encoder, contrastive training, and statistical postprocessing. The method is applied to the Berkeley Earth global land temperature anomaly series \citep{berkeleyearth2024} and compared with PELT based on an L2 mean-shift cost \citep{killick2012}. The goal is not to declare a winner between methods, but to examine what each procedure identifies and how their outputs relate.
...
\subsection{Classical methods: cost functions and PELT}\label{classical-methods-cost-functions-and-pelt}
...
where \(\bar x_{a:b}\) is the segment mean. This cost is small when observations within a segment are close to a common level. A favourable change point therefore separates groups with different means. Related cost functions address changes in variance or other parametric families \citep{truong2020}. The penalty \(\beta\) controls the number of estimated change points: smaller values yield denser segmentations, whereas larger values favour coarser partitions.

An exhaustive search over all possible partitions is computationally expensive. The Pruned Exact Linear Time (PELT) algorithm solves the penalized multiple-change-point problem exactly by combining dynamic programming with a pruning rule \citep{killick2012}. During the recursion, candidate locations that cannot be optimal for any future solution are discarded. Under suitable conditions, the computational cost is linear in \(T\), making PELT practical for long climate records.
...
\subsection{Deep learning methods}\label{deep-learning-methods}

Following \citep{xu2025}, deep CPD replaces hand-crafted statistics with a learned map
...
Let \(X=(x_1,\ldots,x_T)\) denote monthly global land temperature anomalies from Berkeley Earth \citep{berkeleyearth2024}. After removing missing observations, the remaining series is standardized globally,
...
\subsection{Wavelet preprocessing}\label{wavelet-preprocessing}
...
\subsection{Contrastive training}\label{contrastive-training}
...
\subsection{Application to Berkeley Earth Temperature Data}\label{application-to-berkeley-earth-temperature-data}

The Berkeley Earth data contain monthly global land temperature anomalies from 1750 onward \citep{berkeleyearth2024}. Because anomalies are defined relative to a reference climatology, the seasonal cycle has already been removed and no additional seasonal adjustment is applied. Exploratory analysis indicates a long-run increase in the mean, substantially greater variability in the early part of the record, and pronounced temporal dependence. Early observations are particularly unstable and may reflect changes in measurement quality or data coverage in addition to climate variability.

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/07-change_stats/figures/full_time_series} 

}

\caption{Berkeley Earth monthly land temperature anomalies (1753--2023). The blue curve shows the monthly anomaly series, while the orange curve represents a 12-month rolling mean that emphasizes lower-frequency variation. The early part of the record displays substantially greater variability, whereas the late twentieth and early twenty-first centuries are characterized by a pronounced increase in the anomaly level.}\label{fig:cs-full-series}
\end{figure}

\begin{figure}

{\centering \includegraphics[width=1\linewidth]{work/07-change_stats/figures/postprocessing_steps} 

}

\caption{Postprocessing of the raw Wavelet--CNN transition score. Gaussian smoothing with $\sigma=4$ suppresses short-scale fluctuations while retaining broad maxima (top). Applying a minimum height of 0.6 removes weaker peaks (middle). Non-maximum suppression with a minimum temporal separation of 120 months then reduces clusters of nearby peaks to ten final change-point candidates (bottom).}\label{fig:cs-postprocessing-score}
\end{figure}

The ten final candidates and their normalized scores are reported in Table \ref{tab:cs-deep-cp-results}.

\begin{table}

\caption{\label{tab:cs-deep-cp-results}Final Wavelet--CNN change-point candidates and normalized transition scores.}
\centering
\begin{tabular}[t]{rr}
\toprule
Year & Score\\
...
{\centering \includegraphics[width=1\linewidth]{work/07-change_stats/figures/rolling_features} 

}

\caption{Rolling statistical summaries of the temperature anomaly series. From top to bottom, the panels show the 144-month rolling mean, rolling standard deviation, rolling linear trend, and lag-1 autocorrelation. Red dashed lines mark the final Wavelet--CNN candidates. The candidate dates coincide with different combinations of changes in level, variability, trend, and temporal dependence, rather than with a single common statistical feature.}\label{fig:cs-rolling-features}
\end{figure}
...
\tightlist
\item
  \textbf{1763} falls within a period of high early-record variability and a changing local level. Although its score is large, the observational instability of the early instrumental period warrants a cautious interpretation.
\item
  \textbf{1804} coincides with a reversal in the local trend. The rolling mean approaches a local maximum, after which the estimated slope becomes strongly negative.
\item
  \textbf{1818, 1838, and 1851} occur during a prolonged decline in rolling variability accompanied by changes in local trend and dependence. Their distribution over several decades is consistent with stages of an extended transition rather than isolated mean jumps.
\item
  \textbf{1885 and 1941} are associated more clearly with changes in local trend and autocorrelation than with abrupt shifts in the rolling mean. This is consistent with the learned representation responding to aspects of temporal structure beyond the mean level alone.
\item
  \textbf{1960} has no comparably pronounced counterpart in the displayed rolling summaries. The candidate may therefore reflect either a weaker combination of features or characteristics not captured by these four descriptive statistics.
\item
  \textbf{1988} produces the strongest normalized score and lies near a period in which the rolling mean begins to rise persistently. \textbf{1999} occurs within the same broader modern warming phase. Although the ten-year minimum-distance rule permits both candidates to remain, they may represent different parts of one extended transition.
\end{itemize}
...
\section{Comparison of Changepoint Candidates with PELT}\label{comparison-of-changepoint-candidates-with-pelt}
...
{\centering \includegraphics[width=1\linewidth]{work/07-change_stats/figures/comparison_cnn_pelt} 

}

\caption{Change-point candidates identified by the Wavelet--CNN and PELT procedures. Red dashed lines mark Wavelet--CNN candidates, while blue dotted lines mark PELT estimates. Several detections occur in similar periods, whereas others are specific to one procedure, reflecting differences in temporal resolution, model assumptions, and tuning choices.}\label{fig:cs-comparison-cnn-pelt}
\end{figure}

To compare candidate locations, an optimal one-to-one matching is computed with the Hungarian algorithm using a tolerance of 72 months, corresponding to half the CNN window length. Three pairs fall within this tolerance (Table \ref{tab:cs-match-table}).

\begin{table}

\caption{\label{tab:cs-match-table}One-to-one matches between Wavelet--CNN and PELT candidates within a tolerance of 72 months.}
\centering
\begin{tabular}[t]{rrr}
\toprule
...
The application illustrates both the appeal and the limitations of deep CPD in climate statistics. A self-supervised Wavelet--CNN can generate candidate dates without labelled regime shifts, and several neural peaks coincide with visible changes in the rolling mean, standard deviation, slope, or autocorrelation. These observations are consistent with the representation-learning perspective discussed by \citep{xu2025}: once local temporal structure is encoded flexibly, different types of transitions can contribute to a common change score.
...
{\centering \includegraphics[width=0.85\linewidth]{work/09-tp-overview/figures/tp_tipping_point_schematic} 
...
\subsection{Data and Methods}\label{data-and-methods-1}
...
{\centering \includegraphics[width=0.85\linewidth]{work/09-tp-overview/figures/tp_ews_reproduction_plot} 
...
A declining recovery rate therefore also tends to increase variance. Rising \textbf{lag-1 autocorrelation (AC1)} and \textbf{variance} are consequently two of the most widely used statistical indicators of critical slowing down \citep{scheffer2009, dakos2012}.

In practice, these indicators are commonly estimated over rolling windows rather than from the entire time series. To quantify whether an indicator systematically increases over time, this study uses \textbf{Kendall's rank correlation coefficient \(\tau\)} between the indicator and time. A positive value of \(\tau\) indicates an increasing tendency, while values near zero indicate little evidence of a monotonic trend. This provides a simple non-parametric summary of the development of each early-warning indicator.
...
\section{Simulation Benchmark}\label{simulation-benchmark}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Rolling early-warning indicators across the simulation scenarios.}]{book_files/work/08-uncertainty_tp/figures/02_rolling_indicators.png}}
\caption{Rolling early-warning indicators across the simulation scenarios.}\label{fig:utp-rolling-indicators}
\end{figure}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={ROC curves for variance, AC1, and the combined early-warning indicator.}]{book_files/work/08-uncertainty_tp/figures/03_roc_auc.png}}
\caption{ROC curves for variance, AC1, and the combined early-warning indicator.}\label{fig:utp-roc-auc}
\end{figure}

Among the evaluated indicators, \textbf{AC1 shows the strongest discriminatory performance, with an AUC of 0.840}. The combined early-warning measure reaches an AUC of \textbf{0.779}, whereas variance alone produces a lower AUC of \textbf{0.672}. Thus, combining the indicators does not outperform AC1 in this benchmark. These results suggest that increasing persistence contains more useful information about the approaching fold bifurcation than increasing variability alone.

At the selected decision threshold, the empirical sensitivity of the warning signal is \textbf{0.589}, meaning that approximately 58.9\% of tipping simulations generate a positive signal. At the same time, the false-positive rate is \textbf{0.217}, so warning signals also occur in approximately 21.7\% of non-tipping simulations. These values illustrate the central uncertainty associated with EWS: observing a warning signal increases the evidence for an approaching transition, but the signal is neither necessary nor sufficient for tipping.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Frequency of detected warning signals across simulation scenarios.}]{book_files/work/08-uncertainty_tp/figures/04_signal_rates_by_scenario.png}}
\caption{Frequency of detected warning signals across simulation scenarios.}\label{fig:utp-signal-rate}
\end{figure}
...
\section{Bayesian Interpretation of Early-Warning Signals}\label{bayesian-interpretation-of-early-warning-signals}
...
Thus, observing a positive EWS increases the estimated tipping probability from \textbf{10\% to approximately 23.1\%}. The signal therefore contains relevant information, but the posterior probability remains far below certainty. This illustrates why a warning signal should be interpreted as probabilistic evidence rather than as a deterministic diagnosis.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Posterior tipping probability after observing an early-warning signal under different prior probabilities.}]{book_files/work/08-uncertainty_tp/figures/05_bayesian_posterior.png}}
\caption{Posterior tipping probability after observing an early-warning signal under different prior probabilities.}\label{fig:utp-bayesian-posterior}
\end{figure}
...
\section{North Atlantic SST Application}\label{north-atlantic-sst-application}
...
The AMOC is a major component of the climate system and has been discussed as a potential tipping element \citep{lenton2008}. Because sufficiently long direct observations of AMOC strength are not available, previous studies have used sea-surface-temperature (SST) patterns in the North Atlantic as indirect indicators of changes in AMOC stability \citep{boers2021}. Following this general motivation, an SST-based proxy is used here as an illustrative application rather than as a direct measurement of the circulation itself.

The analysis uses annual detrended SST anomalies for the North Atlantic region bounded by \textbf{45--60°N and 60--10°W}, derived from the NOAA Extended Reconstructed Sea Surface Temperature Version 5 (ERSST v5) data set. The early-warning indicators are calculated using a \textbf{20-year rolling window}, applying the same two principal statistics considered in the simulation benchmark: rolling variance and rolling lag-1 autocorrelation (AC1).
...
\section{Declaration of AI Use}\label{declaration-of-ai-use}
...
{\centering \includegraphics[width=0.9\linewidth]{work/11-positive_tp/figures/benchmark_context} 
...
{\centering \includegraphics[width=0.9\linewidth]{work/11-positive_tp/figures/country_context} 
...
{\centering \includegraphics[width=0.9\linewidth]{work/11-positive_tp/figures/spain_scurve} 
...
{\centering \includegraphics[width=0.9\linewidth]{work/11-positive_tp/figures/germany_scurve} 
...
{\centering \includegraphics[width=0.9\linewidth]{work/11-positive_tp/figures/poland_scurve} 
...
{\centering \includegraphics[width=0.9\linewidth]{work/11-positive_tp/figures/fossil_context} 
...
{\centering \includegraphics[width=0.9\linewidth]{work/11-positive_tp/figures/ews_ar} 
...
{\centering \includegraphics[width=0.9\linewidth]{work/11-positive_tp/figures/ews_variance} 
...
\bibliography{bib/cs-bib.bib,bib/ddm-book.bib,bib/nh-bib.bib,bib/ptp-bib.bib,bib/unc-bib.bib,bib/utp-bib.bib,bib/tp-book.bib,bib/dp-bib.bib,bib/irh-book.bib,bib/rh-bib.bib,bib/book.bib,bib/packages.bib}
