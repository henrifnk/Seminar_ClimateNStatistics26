...
The pipeline is outlined in Figure \@ref(fig:downscaling-workflow). In a first step, the original coarse 12 km climate field is bilinearly interpolated to a smoothed version at the target resolution.
...
```{r downscaling-workflow, echo=FALSE, fig.cap="Residual downscaling workflow. The coarse field is bilinearly interpolated, the diffusion model generates the residual, and their sum yields the downscaled field.", out.width="100%", fig.align="center"}
...
the maximum pixel value increases substantially relative to the interpolated field by +28.70 mm/day. Bilinear interpolation had removed part of that peak
...
```{r max-temp-day, echo=FALSE, fig.cap="Day with the highest domain-mean temperature. Left to right: native 12 km grid, bilinear interpolation, downscaled ensemble mean.", out.width="100%", fig.align="center"}
...
kbl(
  ...
  escape    = FALSE,
  caption   = "Field statistics for the hottest day (2080-08-13, temperature in °C) and the wettest day (2056-09-10, precipitation in mm/day). Interpolation is the bilinear interpolation effect (interpolated minus native), Model the diffusion residual (downscaled minus interpolated)."
) |>
