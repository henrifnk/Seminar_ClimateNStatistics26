...
This work addresses both research areas, calibration assessment of the generated fields and projection-signal preservation.
...
but overshoots it. The generated precipitation fields therefore have variance-inflated precipitation values, which is the central shortcoming of the architecture for this variable.
...
The offset is independent of the warming level. This points to the downscaling step being unaffected by the distributional shift.
...
The native 12 km field is mapped onto the fine grid by a nearest-neighbour lookup. Thus, all three fields share one grid.
...
```{r max-temp-day, echo=FALSE, fig.cap="Day with the highest domain-mean temperature. Left: native 12 km grid, middle: bilinear interpolation to the target resolution, right: downscaled ensemble mean.", out.width="100%", fig.align="center"}
...
max_day <- data.frame(
  meta       = c("2080-08-13", "2080-08-13", "2056-09-10", "2056-09-10"),
...
kbl(
  ...
  escape    = FALSE,
  caption   = "Field statistics for the hottest and the wettest day, in °C and mm/day. $\\Delta$ interpolation is the bilinear interpolation effect (interpolated minus native), $\\Delta$ model the diffusion residual (downscaled minus interpolated)."
) |>
  kable_styling(latex_options = "hold_position",
                bootstrap_options = "condensed") |>
  row_spec(0, bold = TRUE, background = "#3f3f3f", color = "white",
