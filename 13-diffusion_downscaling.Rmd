...
**Reanalysis data** is a gridded reconstruction of past weather states and is provided in this work on a resolution of 500 m (1749 × 2198 pixels over the Alpine domain), downscaled from the native CERRA reanalysis.
...
**Projection data** encompasses a simulated trajectory on how Alpine climate might develop over the current century. This work assigned the years to specific Global Warming Levels (GWL) as outlined in Table \@ref(tab:gwl-periods).
...
The reanalysis test consists of every 7th day in the years 2019–2020, totalling 105 days. For every date an ensemble of 12 members was generated. 
...
```{r power-spectrum, echo=FALSE, fig.cap="Radially averaged power spectral density for temperature and precipitation: interpolated field, diffusion model and ground truth.", out.width="100%", fig.align="center"}
knitr::include_graphics("work/13-diffusion_downscaling/figures/calibration/1_power_spectrum.svg")
```

Figure \@ref(fig:spread-skill) addresses the calibration of the ensemble by grouping the pixel-day pairs into 30 equal-count bins by ensemble spread and within each bin calculates the RMSE of the ensemble mean. The spread is corrected for the finite ensemble size [@mardani2025].
...
a difference that is approximately constant across all periods. This points to the downscaling step being unaffected by the distributional shift.

```{r proj-period-means, echo=FALSE, fig.cap="Effect of the downscaling step per global warming level, in percent relative to the interpolated field. Left: mean temperature, right: total precipitation mass.", out.width="100%", fig.align="center"}
...
beforehand (-7.09 mm/day). Thus, a part of the increase only restores the peak that the smoothing had removed. The downscaled maximum is still 11.7 % above the native 12 km maximum.
This may be due to the inflated fine-scale variance reported in Section \@ref(calibration-assessment) which is also evident due to the granular speckle that neither the coarse nor the interpolated field shows.
...
max_day <- data.frame(
  stat       = c("Hottest: domain mean", "Hottest: max pixel",
                 "Wettest: domain mean", "Wettest: max pixel"),
...
A short analysis on whether the spread of the generated ensembles is affected by the distributional shift can be found in [Appendix D](#appendix-d).
