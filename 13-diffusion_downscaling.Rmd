...
Further information on the model parameters and the compute time can be found in [Appendix E](#appendix-e).
...
Figure \@ref(fig:spread-skill) addresses the calibration of the ensemble by grouping the pixel-day pairs into 30 equal-count bins by ensemble spread and calculating the RMSE of the ensemble mean within each bin. 
The spread is corrected for the finite ensemble size following [@mardani2025; @addison2026].
...
### Appendix E - Compute time and model parameters {#appendix-e}
...
#### Model parameters
The most relevant model parameters are listed below. Deviations from the @watt2024 implementation
...
hp <- data.frame(
  ...
  comment = c(
    ...
    "2 noisy residual fields, 2 coarse fields, 2 static fields", "estimated residual fields (tas, pr)",
