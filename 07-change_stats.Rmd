...
bibliography: [cs-bib.bib, packages.bib]
---
...
# Prefer PNG for HTML (bookdown gitbook); PDF for LaTeX/PDF builds.
cs_fig <- function(name) {
  base <- "work/07-change_stats/figures"
  ext <- if (knitr::is_html_output()) "png" else "pdf"
  file.path(base, paste0(name, ".", ext))
}
```
...
knitr::include_graphics(cs_fig("full_time_series"))
```
...
knitr::include_graphics(cs_fig("postprocessing_steps"))
```
...
knitr::include_graphics(cs_fig("rolling_features"))
```
...
knitr::include_graphics(cs_fig("comparison_cnn_pelt"))
```
...
knitr::include_graphics(cs_fig("matched_and_unmatched_change_points"))
```
