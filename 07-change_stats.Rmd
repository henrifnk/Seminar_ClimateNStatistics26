...
bibliography: [cs_bib.bib, packages.bib]
---

```{r cs-setup, message=FALSE, warning=FALSE, include=FALSE}
library(bookdown)
library(svglite)
library(kableExtra)

# SVG for HTML; knitr auto_pdf uses the identically named PDF in LaTeX.
cs_fig <- function(name) {
  file.path("work/07-change_stats/figures", paste0(name, ".svg"))
}
```
...
Let \(X=(x_1,\ldots,x_T)\) denote monthly global land temperature anomalies from Berkeley Earth [@berkeleyearth2024]. After removing missing observations, the remaining series is standardized globally,
...
```{r cs-match-table, echo=FALSE}
match_table <- data.frame(
  `CNN year` = c(1804, 1818, 1999),
  `PELT year` = c(1808, 1818, 2001),
  `Difference (months)` = c(48, 7, 25),
  check.names = FALSE
)

knitr::kable(
  match_table,
  caption = "One-to-one matches between Wavelet--CNN and PELT candidates within a tolerance of 72 months.",
  booktabs = TRUE
)
```
